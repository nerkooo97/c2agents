...
const allTools: Tool<any, any>[] = [
  ...
  webSearch,
  // playwright,
].filter(Boolean); // Filter out any disabled (undefined) tools


// The promise now resolves immediately with the static list.
const allToolsPromise = Promise.resolve(allTools);
